...
import { FailKnowledgeBaseReembeddingCommandHandler } from './fail-knowledge-base-reembedding.handler';

describe('FailKnowledgeBaseReembeddingCommandHandler', () => {
  it('sets embeddingStatus to FAILED and saves', async () => {
    const now = new Date();
    ...
    const handler = new FailKnowledgeBaseReembeddingCommandHandler(
      writeRepository,
      assertExists,
      eventBus,
    );
    ...
    await handler.execute(
      new FailKnowledgeBaseReembeddingCommand({
